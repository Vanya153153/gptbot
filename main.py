...
# Сценарист:
# 1. При сдаче проекта представлена документация с описанием проекта
# 2. Представлены не менее трёх жанров рассказов на выбор. Например, хоррор, юмор и фэнтези
# 3. Представлены персонажи женского и мужского пола на выбор, минимум по два варианта на каждый пол
# 4. Подготовлено описание не менее трёх сеттингов — литературных, кино- или игровых вселенных

# Реализация бота:
# 1. Оформлен профиль бота: имя, описание, информация, аватарка и команды
# 2. Варианты ответа пользователя реализованы в виде кнопок
# 3. Задано ограничение на количество пользователей и длительности сессий
# (пользователей должно быть минимум два — разработчик и наставник)

# API нейросети:
# 1. Пользователь может включить отладочный режим в боте
# 2. Пользователь в отладочном режиме получает сообщения о статусах запросов к нейросети

# привет :) в этом видео я хочу сама попытаться написать бота-сценариста по ТЗ, которое описано выше
# некоторая информация здесь является не слишком полезной для написания бота,
# так как его оформление, например, - не сама полезная задача
# теперь я попробую добавить некий отладочный режим
# ух, кажется, теперь это похоже на то, что требуется по ТЗ. Удачи и вам!)
